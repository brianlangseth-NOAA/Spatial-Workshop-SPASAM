...
remove_years <- function(no_yrs){
  ...
  em_dat[(loc+1)] <- shorten_line(em_dat, loc, no_yrs)
  
  #OBS_survey_fleet_bio
  loc <- grep("#OBS_survey_fleet_bio$", em_dat)
  em_dat <- shorten_vector(em_dat, loc, no_yrs, orig_yrs)
  ...
  #T - length is years*ages
  #No function for this so apply manually
  loc <- grep("#T$", em_dat)
  tmp <- em_dat[(loc+1):(loc+(orig_yrs*orig_ages))]
  em_dat <- em_dat[-c((loc+1):(loc+(orig_yrs*orig_ages)))]
  em_dat <- append(em_dat, tmp[-c(1:(no_yrs*orig_ages))], loc)
...
#Entries with years all within one line
shorten_line <- function(datfile, var_loc, n_yrs) {
  
  unlisted <- unlist(strsplit(datfile[(var_loc+1)], split = " "))
  if(unlisted[1]=="") unlisted = unlisted[-1] #remove the empty space in first element if data was indented
  shortened <- paste(unlisted[-c(1:n_yrs)], collapse = " ")
  return(shortened)
  
} 

#Entries with years on each line
shorten_vector <- function(datfile, var_loc, n_yrs, o_yrs, prop = FALSE) {
  ...
  if(!prop) { #if not proportions length is yrs
    unlisted <- datfile[(var_loc+1):(var_loc+o_yrs)] 
    datfile <- datfile[-c((var_loc+1):(var_loc+o_yrs))]
  }
  if(prop) { #if proportions length is yrs*flt
    unlisted <- datfile[(var_loc+1):(var_loc+(o_yrs*o_flt))] 
    datfile <- datfile[-c((var_loc+1):(var_loc+(o_yrs*o_flt)))] 
  }
  
  shortened <- unlisted[-c(1:n_yrs)] #remove first n_yrs of data
