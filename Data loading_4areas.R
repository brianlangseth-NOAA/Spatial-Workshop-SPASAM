...
badruns = c(3,6,9,11,13,16,17,19,21,22,25,35,40,42,43,46,48,49,50)
badruns = c(13,16,17,19,21,22,25,35,40,42,43,46,48,49,50)
for(i in badruns){
  dat <- get(paste0("dat_4A_",i))
  mod_name <- paste0("YFT_4area_100sets_18alk_",i)
  om_rep <- mungeData(mod_name, reduce = 105, run = TRUE, fleetcombo = TRUE, remove_regions = c(2,3), tpl = "fixF", move = "final")
...
#The 0.9 age 16 fish caught in first year of release is less than...the one recaught in second year
  ...
  tmp_val[tmp_index[1],,]
  if(nrow(tmp_index)>0){ #do only if recaps are more than releases
    for(i in 1:nrow(tmp_index)){
      tmp_recap <- which(tmp_val[tmp_index[i,1],,tmp_index[i,2]] > 0, arr.ind = TRUE) #year of recapture of released fish
      tmp_val[tmp_index[i,1], c(tmp_recap,max(nt)), tmp_index[i,2]] <- c(tmp_val[tmp_index[i,1],tmp_recap,tmp_index[i,2]] + tmp_val[tmp_index[i,1],max(nt),tmp_index[i,2]]/length(tmp_recap), 0)
    }
  }
